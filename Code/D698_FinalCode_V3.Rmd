...
g2<-c_data%>%ggplot(aes(y=SWND_EVENTS,x=factor(WFRI_R)))+geom_boxplot()+theme_light()+labs(x="WildFire Present",y="Annual Strong Wind Events")
...
```{r eval=FALSE}
automl.pred<-h2o.predict(aml@leader,test.h2o)%>%as.data.frame()%>%pull(predict)
automl.precsnprob<-h2o.predict(aml@leader,test.h2o)%>%as.data.frame()%>%pull(p1)
...
```{r eval=FALSE}
automl.perf<-h2o.performance(winining_aml,test.h2o)%>%h2o.metric()%>%as.data.frame()%>%select(c(recall,precision))
...
test.results[5,]<-c(R2=temp$R2,MSE=temp$MSE,RMSE=temp$RMSE,AUC=temp$AUC,classError=temp$classError)
rownames(test.results)<-c("Random Forest","Gradient Boosting","automl- stack ensemble","Navie Bayes","Deep Learning")
